...
def run_prodigal(file_name):
    ...
    with open(prodigal_out, 'w') as prodigal_out_file:
        ...
        cds_file = os.path.splitext(file_name)[0] + '.cds'
        subprocess.run(['prodigal', '-i', fasta_file, '-a', cds_file, '-p', 'meta', '-q'], stdout=prodigal_out_file)
    ...
    with open(args.output_file, 'w') as output_file:
        ...
        with open(args.list, 'r') as input_list:
            ...
            for file_name in input_list:
                ...
                cds_file_name = os.path.splitext(file_name)[0] + '.cds'
                ...
                run_pfamscan(cds_file_name, database_path)
                pfamscan_out = os.path.splitext(cds_file_name)[0] + '.pfamscan'
                ...
                output_file.write(f"# Query: {file_name}\n# Domains: {query_pfams}\n# Top {n} matches for {file_name} ({sequence_length} bp, {n_cds} CDS):\n")
                print(f"Top {n} matches for {file_name}:")
                output_file.write(f"# Profile Name\tKmer Dissimilarity\tKmer Similarity\tLength\tLength Proportion\tCDS count\tCDS proportion\tUnion Domains\tIntersection Domains\tDomain count dissimilarity\tDomain count similarity\tOverall score\n")
                ...
                for match in top_n_matches:
                    ...
                    output_fasta_name = f"{accession}.fasta"
                    ...
                    cds_file_name = os.path.splitext(output_fasta_name)[0] + '.cds'
                    if os.path.exists(cds_file_name):
                        print(f"{cds_file_name} already exists. Moving forward...")
                    else:
                    ...
                    if cds_count == n_cds:
                        ...
                        minimum_value = min(cds_count, n_cds)
                        maximum_value = max(cds_count, n_cds)
                        cds_proportion = minimum_value / maximum_value                         
                    ...
                    pfamscan_out_file = os.path.splitext(cds_file_name)[0] + '.pfamscan'
                    ...
                    average_score = (similarity + length_proportion + cds_proportion + bray_curtis_similarity)/4

                    result = (profile_name, dissimilarity, similarity, fasta_length, length_proportion, cds_count, cds_proportion,  union_domains, intersection_domains, bray_curtis_dissimilarity, bray_curtis_similarity, average_score)
